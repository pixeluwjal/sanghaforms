...
const getIcon = (type: string) => {
  switch (type) {
    ...
    case "sangha":
      return Users;
    default:
      return File; // Default fallback icon
  }
};
...
export default function FieldPreview({ field }: FieldPreviewProps) {
  ...
  const commonInputClasses =
    "w-full px-4 py-3 border-2 border-slate-200 rounded-xl bg-white text-base text-slate-700 shadow-lg shadow-slate-50/50 transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-indigo-100 placeholder:text-slate-400 disabled:bg-slate-50 disabled:cursor-not-allowed";
  ...
  const FieldWrapper = ({ children }: { children: React.ReactNode }) => (
    <div className="mb-6">
      ...
      {children}
      <p className="text-xs text-slate-400 mt-1 italic opacity-80">
        {field.placeholder || `Preview of ${field.type} field.`}
      </p>
    </div>
  );
  ...
  switch (field.type) {
        ...
        <FieldWrapper>
          ...
          >
            ...
            {field.options?.map((option, idx) => (
              <option key={idx} className="text-slate-800">
                {option}
              </option>
            ))}
          </select>
    ...
    case "date":
      return (
        <FieldWrapper>
          <input type="date" className={commonInputClasses} disabled />
        </FieldWrapper>
      );
          ...
          <div className="space-y-4 bg-purple-50 rounded-xl p-5 border-2 border-purple-200 shadow-md">
            <div className="flex items-center gap-3 text-purple-700">
              ...
              <span className="text-base font-extrabold tracking-wide">
                Sangha Hierarchy Selector
              </span>
            </div>
            <p className="text-sm text-purple-600 mb-3">
              This field links to the organizational structure.
            </p>
        ...
        <FieldWrapper>
          <div className="border-4 border-dashed border-indigo-300 rounded-2xl p-8 text-center bg-white hover:border-indigo-500 transition-all duration-300 shadow-xl cursor-not-allowed">
            ...
            <p className="text-sm text-slate-500 mt-1">
              Maximum file size: 10MB (Preview Mode)
            </p>
          </div>
        </FieldWrapper>
      );
