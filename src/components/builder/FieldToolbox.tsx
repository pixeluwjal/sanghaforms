  ...
  return (
    <div className="bg-white/90 backdrop-blur-sm rounded-2xl border border-white/20 shadow-xl p-4 sm:p-6 h-fit lg:sticky lg:top-6 max-h-[calc(100vh-2rem)] overflow-y-auto">
      <div className="text-center mb-4 sm:mb-6">
        <div className={`inline-flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 bg-gradient-to-r ${purpleConfig.gradient} rounded-full text-white mb-2 sm:mb-3`}>
        ...
        {FIELD_TYPES.map((fieldType) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-2 sm:gap-4 p-3 sm:p-4 ${purpleConfig.bg} ${purpleConfig.border} border-2 rounded-xl transition-all duration-300 hover:scale-105 hover:shadow-lg group text-left`}
            >
              <div className={`w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 rounded-xl ${purpleConfig.bg} border-2 ${purpleConfig.border} flex items-center justify-center group-hover:scale-110 transition-transform duration-300 shadow-inner flex-shrink-0`}>
                <IconComponent className={`w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 ${purpleConfig.text}`} />
              </div>
              ...
              <Plus className={`w-3 h-3 sm:w-4 sm:h-4 lg:w-5 lg:h-5 ${purpleConfig.text} opacity-0 group-hover:opacity-100 transition-all duration-300 transform group-hover:rotate-90 flex-shrink-0`} />
            </button>
          );
        })}
      </div>
      ...
      {/* Quick Tips */}
      <div className="mt-4 sm:mt-6 p-3 sm:p-4 bg-gradient-to-r from-purple-50 to-purple-100 rounded-xl border border-purple-200">
